...
if [ -d $CKAN_DATADIR ]; then
    ...
    echo "INICIALIZANDO..."
    echo "*******************************************************"
    cd /opt/app && git clone --depth=1 -b $CKAN_VERSION $CKAN_REPO  && cd /opt/app/ckan && pip install -e . && \
    ...
    paster --plugin=ckan user add admin  email=admin@admin.com password=adminadmin --config=/opt/app/ckan/config/production.ini && \
    paster --plugin=ckan sysadmin add admin --config=/opt/app/ckan/config/production.ini  && \
echo "******************************************"
